...
def _detect_peer_name(conn, conv_id: str, conv_name: str) -> str:
    """返回会话对方的真实抖音昵称。

    策略（按可靠性排序）：
    1. conv_id 格式 '0:1:uidA:uidB'：取 uidB（单聊时 uidB 通常是对方），
       查 users.nickname；若 uidB 查不到，尝试 uidA
    2. 从该会话消息中找非"自己"的发送者昵称（排除与 conv_name 相同的 sender）
    3. 兜底：conv_name 原样
    """
    # 策略 1: 解析 conv_id 里的 uid
    parts = str(conv_id).split(":")
    if len(parts) >= 4:
        for uid in (parts[3], parts[2]):  # 先 uidB 后 uidA
            if not uid or not uid.isdigit():
                continue
            row = conn.execute(
                "SELECT nickname FROM users WHERE uid = ?", (uid,)
            ).fetchone()
            if row and row[0] and str(row[0]).strip():
                return str(row[0]).strip()
    # 策略 2: 从消息发送者找"不是会话名"的那个昵称
    rows = conn.execute(
        """SELECT DISTINCT sender_uid FROM messages WHERE conv_id = ? LIMIT 5""",
        (conv_id,),
    ).fetchall()
    candidates = []
    for (uid,) in rows:
        ...
        nick = str(row[0]).strip() if row and row[0] else ""
        if nick and nick != str(conv_name).strip():
    ...
    if candidates:
        ...
        for nick in candidates:
            n = conn.execute(
                "SELECT COUNT(*) FROM messages WHERE conv_id = ? AND sender_uid IN "
                "(SELECT uid FROM users WHERE nickname = ?)",
                (conv_id, nick),
            ).fetchone()[0]
            if n > best_n:
                best, best_n = nick, n
        if best:
            return best
    return str(conv_name or "").strip() or "对话"


class ChatLabExporter:
    def __init__(self, conv_name: str = None, output_format: str = "jsonl", conv_id: str = None):
        self.conv_name = conv_name
        self.output_format = output_format  # "json" or "jsonl"
        self.conv_id = conv_id  # 精确会话 ID（优先于 conv_name，解决同名会话）
    ...
    def export(self, output_path: str):
        ...
        # Find conversation: 优先精确 conv_id，其次按名字模糊（兼容旧调用）
        if self.conv_id:
            row = conn.execute(
                "SELECT conv_id, name FROM conversations WHERE conv_id = ?",
                (self.conv_id,),
            ).fetchone()
            if not row:
                print(f"[-] 未找到会话 conv_id: {self.conv_id}")
                conn.close()
                return
        elif self.conv_name:
            row = conn.execute(
        ...
        conv_id = row["conv_id"]
        conv_name = row["name"]
        # 真实抖音名：对方昵称（导出的 meta.name / 文件名都用它）
        peer_name = _detect_peer_name(conn, conv_id, conv_name)
        print(f"[*] 导出会话: {conv_name} (真实对方: {peer_name}) (ID: {conv_id})")
